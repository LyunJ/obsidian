...
스토어드 프로그램에서 핸들러를 정의할 때 에러 번호로 핸들러를 정의할 수도 있다. 하지만 이때 똑같은 원인에 대해 여러개의 에러 번호를 가지는 경우 에러 번호 중 하나라도 빠뜨리면 제대로 에러 행들링을 못할 수도 있다.
...
### 시그널을 이용한 예외 발생
#### 시그널
예외나 에러에 대한 핸들링이 있다면 반대로 예뢰를 사용자가 직접 발생시킬 수 있는 기능이 있어야 할 것이다.
